...
class BeamformingTemporalVAEModel(Model):
    def __init__(self, num_antennas, num_users, num_slots):
        super().__init__()
        self.num_antennas = num_antennas
        self.num_users = num_users
        self.num_slots = num_slots
        self.lstm = layers.GRU(512, return_sequences=True, kernel_regularizer=tf.keras.regularizers.L2(0.001))
        self.temporal_attention = layers.MultiHeadAttention(num_heads=4, key_dim=64)
        self.norm = layers.LayerNormalization()
        self.dense1_real = layers.Dense(256, activation="relu")
        self.dense1_imag = layers.Dense(256, activation="relu")
        self.dense2 = layers.Dense(128, activation="relu")
        self.output_layer = layers.Dense(num_antennas * num_users)
        self.omega = {}
        self.old_params = {}
        self.task_params = {}
    ...
    def call(self, inputs):
        ...
        slots_per_batch = tf.shape(inputs)[1]
        ...
        real_seq = self.lstm(real_inputs)
        imag_seq = self.lstm(imag_inputs)
        combined_seq = tf.stack([real_seq, imag_seq], axis=2)
        combined_seq = tf.reshape(combined_seq, [batch_size, slots_per_batch, 2 * 512])
        attn_output = self.temporal_attention(combined_seq, combined_seq)
        attn_output = self.norm(attn_output)
        x = tf.reduce_mean(attn_output, axis=1)
        real_x = x[:, :512]
        imag_x = x[:, 512:]
        real_x = self.dense1_real(real_x)
        imag_x = self.dense1_imag(imag_x)
        x = tf.concat([real_x, imag_x], axis=-1)
        x = self.dense2(x)
        output = self.output_layer(x)
        real_output = tf.cast(output, tf.float32)
        imag_output = tf.cast(output, tf.float32)
        w = tf.complex(real_output, imag_output)
        w = tf.reshape(w, [batch_size, 1, self.num_users, self.num_antennas])
    ...
    def save_task_params(self, task_idx):
        self.task_params[f"task_{task_idx}"] = {w.name: w.numpy() for w in self.trainable_weights}
...
def train_step(model, x_batch, h_batch, optimizer, task_idx, replay_buffer=None):
    with tf.GradientTape() as tape:
        ...
        if replay_buffer and len(replay_buffer[0]) > 0:
            replay_idx = tf.random.uniform(shape=[min(BATCH_SIZE, len(replay_buffer[0]))], 
                                          minval=0, maxval=len(replay_buffer[0]), dtype=tf.int32)
            ...
            if replay_x.shape[1] != NUM_SLOTS:
                replay_x = tf.tile(replay_x[:, :1, :, :], [1, NUM_SLOTS, 1, 1])
                replay_h = tf.tile(replay_h[:, :1, :, :], [1, NUM_SLOTS, 1, 1])
...
def main(seed):
    ...
    model = BeamformingTemporalVAEModel(NUM_ANTENNAS, NUM_USERS, NUM_SLOTS)
    dummy_input = tf.zeros((BATCH_SIZE, NUM_SLOTS, NUM_ANTENNAS, NUM_USERS), dtype=tf.complex64)
    model(dummy_input)
    
    total_start_time = time.time()
    
    # Pretraining
    pretrain_optimizer = tf.keras.optimizers.Adam(learning_rate=0.002)
    print("Pretraining model on mixed tasks...")
    mixed_h = []
    for task in TASKS:
        h = generate_channel(task, NUM_SLOTS // len(TASKS))
        mixed_h.append(h)
    mixed_h = tf.concat(mixed_h, axis=0)
    mixed_dataset = tf.data.Dataset.from_tensor_slices((mixed_h, mixed_h)).shuffle(1000).batch(BATCH_SIZE)
    for epoch in range(2):
        for x_batch, h_batch in mixed_dataset:
            loss = train_step(model, x_batch, h_batch, pretrain_optimizer, task_idx=0)
    print("Pretraining completed")
    
    # Training loop for tasks
    for task_idx, task in enumerate(TASKS):
        ...
        replay_buffer = [[], []]
        current_indices = tf.random.shuffle(tf.range(tf.shape(x)[0]))[:REPLAY_BUFFER_SIZE]
        replay_samples_x = tf.gather(x, current_indices)
        replay_samples_h = tf.gather(h, current_indices)
        replay_buffer[0] = replay_samples_x.numpy()
        replay_buffer[1] = replay_samples_h.numpy()
        
        optimizer = tf.keras.optimizers.Adam(
            learning_rate=tf.keras.optimizers.schedules.ExponentialDecay(
                initial_learning_rate=0.002 if task_idx == 0 else 0.01,
                decay_steps=1000,
                decay_rate=0.96,
                staircase=True
            )
        ...
        if task_idx > 0:
            ...
            print(f"Task {task['name']} (idx {task_idx}): initial_throughput = {initial_throughput:.4f}, past_throughput_before = {past_throughput_before:.4f}, FWT = {fwt:.4f}")
        else:
            fwt = 0.0
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for x_batch, h_batch in dataset:
                loss = train_step(model, x_batch, h_batch, optimizer, task_idx, replay_buffer)
                epoch_loss += loss
                batches += 1
        ...
        if task_idx > 0:
            ...
            for past_idx in range(task_idx):
                ...
                forgetting_delta = past_throughput - task_initial_performance[past_idx]
        ...
        weights_dir = os.path.join("saved_models", f"model_task_{task['name']}_seed_{seed}")
...
if __name__ == "__main__":
    ...
    with open("results_summary.txt", 'w') as f:
        ...
        plt.title("Forgetting Across Mobility Tasks (Mean ± Std)")
        plt.grid(True)
        plt.ylim(-2.5, 0)
        plt.xticks(range(len(TASKS)), [task["name"] for task in TASKS], rotation=45)
        plt.legend()
        ...
        plt.xticks(range(len(cl_metrics)), list(cl_metrics.keys()))
        plt.grid(True)
        plt.tight_layout()
